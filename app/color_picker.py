...
def get_best_color(photo):
   opened_photo = Image.open(photo)
   photo_ndarray = np.asarray(opened_photo)
   rgb_list = [math.ceil(i) for i in SpotifyBackgroundColor(photo_ndarray, image_processing_size=(100, 100)).best_color()]
   rgb_string = f'rgb({rgb_list[0]}, {rgb_list[1]}, {rgb_list[2]})'
   return rgb_string